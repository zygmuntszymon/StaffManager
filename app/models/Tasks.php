...
class Tasks {
    ...
    public function markTaskAsDone($taskId, $userId) {
        $stmt = $this->pdo->prepare("
            UPDATE zadania 
            SET status = 'ukończone', data_zakonczenia = CURDATE() 
            WHERE id = ?
        ");
    ...
    public function getTasksForUser($userId) {
        $stmt = $this->pdo->prepare("
            SELECT z.id, z.opis, z.status, z.deadline, z.ilosc_punkty, z.data_zakonczenia
            FROM zadania z
            JOIN pracownik_zadanie pz 
            ON pz.zadanie_id = z.id
    ...
    public function getAllTasks()
    {
        $stmt = $this->pdo->prepare("
            SELECT * FROM zadania 
            ORDER BY opis, deadline, ilosc_punkty, status
            ");
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function addTask($opis, $deadline, $ilosc_punkty, $status) {
        $stmt = $this->pdo->prepare("
            INSERT INTO zadania (opis, status, deadline, ilosc_punkty) 
            VALUES (?, ?, ?, ?)
            ");
    ...
    public function taskForUser($pracownik_id, $zadanie_id) {
        ...
        $stmt = $this->pdo->prepare("
            INSERT INTO pracownik_zadanie (pracownik_id, zadanie_id) 
            VALUES (?, ?)
            ");
    ...
    public function updateTask($id, $opis, $deadline, $ilosc_punkty)
    {
            $stmt = $this->pdo->prepare("
                UPDATE zadania 
                SET opis = ?, deadline = ?, ilosc_punkty = ? 
                WHERE id = ?
                ");
    ...
    public function getNameAndSurnameWorker($zadanie_id) {
        $stmt = $this->pdo->prepare("
            SELECT CONCAT(p.imie, ' ', p.nazwisko) AS nameAndSurname
